...
def ThinPlateSpline(U, coord, vector, out_size, device, move=None, scal=None):
    ...
    output = torch.reshape(input_transformed, [num_batch, out_height, out_width, channels]).permute(0, 3, 1, 2)
...
def ThinPlateSpline2(U, coord, vector, out_size, device, move=None, scal=None):
    coord = torch.flip(coord, [2])
    vector = torch.flip(vector, [2])
