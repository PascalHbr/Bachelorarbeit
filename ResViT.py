...
class ViTResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=256, dim=256, num_tokens=18, mlp_dim=256, heads=8, depth=6,
        ...
        self.bn1 = nn.InstanceNorm2d(18)
        self.bn2 = nn.InstanceNorm2d(18)
        self.up_Conv1 = nn.Sequential(nn.ConvTranspose2d(in_channels=18, out_channels=18, kernel_size=4,
                                                         stride=2, padding=1),
                                      self.bn1,
                                      self.relu)

        self.up_Conv2 = nn.Sequential(nn.ConvTranspose2d(in_channels=18, out_channels=18, kernel_size=4,
                                                         stride=2, padding=1),
                                      self.bn2,
                                      self.relu)
    ...
    def forward(self, img, mask=None):
        ...
        # Tokenization
        bn = x.shape[0]
        wa = rearrange(self.token_wA.repeat(bn, 1, 1), 'b h w -> b w h')  # Transpose
        A = torch.einsum('bij,bjk->bik', x, wa)
        A = rearrange(A, 'b h w -> b w h')  # Transpose
        A = A.softmax(dim=-1)

        VV = torch.einsum('bij,bjk->bik', x, self.token_wV.repeat(bn, 1, 1))
